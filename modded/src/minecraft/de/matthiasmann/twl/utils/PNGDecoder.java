...
public class PNGDecoder {
	...
	public PNGDecoder.Format decideTextureFormat(PNGDecoder.Format fmt) {
		...
		case 0:
			switch($SWITCH_TABLE$de$matthiasmann$twl$utils$PNGDecoder$Format()[fmt.ordinal()]) {
			case 1:
			case 2:
				return fmt;
			default:
	...
	private void copyRGBtoABGR(ByteBuffer buffer, byte[] curLine) {
		if(this.transPixel != null) {
			byte i = this.transPixel[1];
			byte n = this.transPixel[3];
			byte tb = this.transPixel[5];
			int i1 = 1;

			for(int n1 = curLine.length; i1 < n1; i1 += 3) {
				byte r = curLine[i1];
				byte g = curLine[i1 + 1];
				byte b = curLine[i1 + 2];
				byte a = -1;
			...
			for(int n2 = curLine.length; i2 < n2; i2 += 3) {
				buffer.put((byte)-1).put(curLine[i2 + 2]).put(curLine[i2 + 1]).put(curLine[i2]);
			}
		}
	...
	private void copyRGBtoRGBA(ByteBuffer buffer, byte[] curLine) {
		if(this.transPixel != null) {
			...
			for(int n2 = curLine.length; i2 < n2; i2 += 3) {
				buffer.put(curLine[i2]).put(curLine[i2 + 1]).put(curLine[i2 + 2]).put((byte)-1);
			}
		}
	...
	private void copyRGBtoBGRA(ByteBuffer buffer, byte[] curLine) {
		if(this.transPixel != null) {
			...
			for(int n2 = curLine.length; i2 < n2; i2 += 3) {
				buffer.put(curLine[i2 + 2]).put(curLine[i2 + 1]).put(curLine[i2]).put((byte)-1);
			}
		}
	...
	private void readChunkUnzip(Inflater inflater, byte[] buffer, int offset, int length) throws IOException {
		...
		try {
			do {
				int ex = inflater.inflate(buffer, offset, length);
				if(ex <= 0) {
			...
			throw (IOException)(new IOException("inflate error")).initCause(dataFormatException6);
		}
	}
	...
	static int[] $SWITCH_TABLE$de$matthiasmann$twl$utils$PNGDecoder$Format() {
		...
		if($SWITCH_TABLE$de$matthiasmann$twl$utils$PNGDecoder$Format != null) {
			...
			int[] i0 = new int[PNGDecoder.Format.values().length];

			try {
				i0[PNGDecoder.Format.ABGR.ordinal()] = 7;
			} catch (NoSuchFieldError noSuchFieldError7) {
			}

			try {
				i0[PNGDecoder.Format.ALPHA.ordinal()] = 1;
			} catch (NoSuchFieldError noSuchFieldError6) {
			}

			try {
				i0[PNGDecoder.Format.BGRA.ordinal()] = 6;
			} catch (NoSuchFieldError noSuchFieldError5) {
			}

			try {
				i0[PNGDecoder.Format.LUMINANCE.ordinal()] = 2;
			} catch (NoSuchFieldError noSuchFieldError4) {
			}

			try {
				i0[PNGDecoder.Format.LUMINANCE_ALPHA.ordinal()] = 3;
			} catch (NoSuchFieldError noSuchFieldError3) {
			}

			try {
				i0[PNGDecoder.Format.RGB.ordinal()] = 4;
			} catch (NoSuchFieldError noSuchFieldError2) {
			}

			try {
				i0[PNGDecoder.Format.RGBA.ordinal()] = 5;
