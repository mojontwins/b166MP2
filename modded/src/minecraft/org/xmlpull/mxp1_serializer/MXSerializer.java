...
public class MXSerializer implements XmlSerializer {
	...
	public boolean getFeature(String name) throws IllegalArgumentException {
		if(name == null) {
			...
			return "http://xmlpull.org/v1/doc/features.html#names-interned".equals(name) ? this.namesInterned : ("http://xmlpull.org/v1/doc/features.html#serializer-attvalue-use-apostrophe".equals(name) ? this.attributeUseApostrophe : false);
		}
	}

	protected void rebuildIndentationBuf() {
		if(this.doIndent) {
			boolean maxIndent = true;
			int bufSize = 0;
	...
	public Object getProperty(String name) throws IllegalArgumentException {
		if(name == null) {
			...
			return "http://xmlpull.org/v1/doc/properties.html#serializer-indentation".equals(name) ? this.indentationString : ("http://xmlpull.org/v1/doc/properties.html#serializer-line-separator".equals(name) ? this.lineSeparator : ("http://xmlpull.org/v1/doc/properties.html#location".equals(name) ? this.location : null));
		}
	}
	...
	public void startDocument(String encoding, Boolean standalone) throws IOException {
		boolean apos = this.attributeUseApostrophe ? true : true;
		...
		if(this.out == null) {
			...
			if(namespace != null) {
				...
				if(namespace.length() > 0) {
					...
					if(i == null) {
						i = this.lookupOrDeclarePrefix(namespace);
					}

					if(i.length() > 0) {
						this.elPrefix[this.depth] = i;
						this.out.write(i);
						this.out.write(58);
					} else {
					...
					for(int i9 = this.namespaceEnd - 1; i9 >= 0; --i9) {
						if(this.namespacePrefix[i9] == "") {
							uri = this.namespaceUri[i9];
							if(uri == null) {
								this.setPrefix("", "");
	...
	protected void writeAttributeValue(String value, Writer out) throws IOException {
		...
		for(int i = 0; i < value.length(); ++i) {
			...
			if(ch == 60) {
				...
				if(ch != 13 && ch != 10 && ch != 9) {
					throw new IllegalStateException("character " + printable(ch) + " (" + Integer.toString(ch) + ") is not allowed in output" + this.getLocation() + " (attr value=" + printable(value) + ")");
				}
		...
		for(int i = 0; i < text.length(); ++i) {
			...
			if(ch == 93) {
				...
				if(ch == 38) {
					...
					out.write("&lt;");
					pos = i + 1;
				} else if(this.seenBracketBracket && ch == 62) {
					if(i > pos) {
						out.write(text.substring(pos, i));
					}
					...
					throw new IllegalStateException("character " + Integer.toString(ch) + " is not allowed in output" + this.getLocation() + " (text value=" + printable(text) + ")");
				}
		...
		for(int i = off; i < end; ++i) {
			...
			if(ch == 93) {
				...
				if(ch == 38) {
					...
					throw new IllegalStateException("character " + printable(ch) + " (" + Integer.toString(ch) + ") is not allowed in output" + this.getLocation());
				}
