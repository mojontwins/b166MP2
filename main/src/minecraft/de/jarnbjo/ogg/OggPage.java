...
public class OggPage {
	...
	private OggPage(int version, boolean continued, boolean bos, boolean eos, long absoluteGranulePosition,
			int streamSerialNumber, int pageSequenceNumber, int pageCheckSum, int[] segmentOffsets,
			int[] segmentLengths, int totalLength, byte[] header, byte[] segmentTable, byte[] data) {
	...
	/**
	 * This method is called to read data from the current position in the specified
	 * RandomAccessFile and create a new OggPage instance based on the data read. If
	 * the parameter <code>skipData</code> is set to <code>true</code>, the actual
	 * page segments (page data) is skipped and not read into memory. This mode is
	 * useful when scanning through an ogg file to build a seek table.
	 *
	 * @param source   the source from which the ogg page is generated
	 * @param skipData if set to <code>true</code>, the actual page data is not read
	 *                 into memory
	 * @return an ogg page created by reading data from the specified source,
	 *         starting at the current position
	 * @throws FormatException      if the data read from the specified source is
	 *                              not matching the specification for an ogg page
	 * @throws EndOfStreamException if it is not possible to read an entire ogg page
	 *                              from the specified source
	 * @throws IOException          if some other I/O error is detected when reading
	 *                              from the source
	 *
	...
	/**
	 * This method is called to read data from the current position in the specified
	 * InpuStream and create a new OggPage instance based on the data read. If the
	 * parameter <code>skipData</code> is set to <code>true</code>, the actual page
	 * segments (page data) is skipped and not read into memory. This mode is useful
	 * when scanning through an ogg file to build a seek table.
	 *
	 * @param source   the source from which the ogg page is generated
	 * @param skipData if set to <code>true</code>, the actual page data is not read
	 *                 into memory
	...
	/**
	 * This method is called to create a new OggPage instance based on the specified
	 * byte array.
	 *
	 * @param source   the source from which the ogg page is generated
	 * @param skipData if set to <code>true</code>, the actual page data is not read
	...
	private static OggPage create(Object source, boolean skipData)
		...
		try {
			...
			if (capture != 0x5367674f) {
				// throw new FormatException("Ogg page does not start with 'OggS'
				// (0x4f676753)");

				/*
				 ** This condition is IMHO an error, but older Ogg files often contain pages with
				 * a different capture than OggS. I am not sure how to manage these pages, but
				 * the decoder seems to work properly, if the incorrect capture is simply
				 * ignored.
				 */
				...
				System.out.println(
						"Ogg packet header is 0x" + cs + " (" + c1 + c2 + c3 + c4 + "), should be 0x4f676753 (OggS)");
			}
			...
			return new OggPage(version, bf1, bos, eos, absoluteGranulePosition, streamSerialNumber, pageSequenceNumber,
					pageCheckSum, segmentOffsets, segmentLengths, totalLength, header, segmentTable, data);
	...
	/**
	 * Returns the absolute granule position of the last complete packet contained
	 * in this Ogg page, or -1 if the page contains a single packet, which is not
	 * completed on this page. For pages containing Vorbis data, this value is the
	 * sample index within the Vorbis stream. The Vorbis stream does not necessarily
	 * start with sample index 0.
	 *
	 * @return the absolute granule position of the last packet completed on this
	 *         page
	 */
