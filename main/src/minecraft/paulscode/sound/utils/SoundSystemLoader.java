 ...
 * via external setup files. <br>
 * <br>
 * <b><i> SoundSystemLoader License:</b></i><br>
 * <b><br>
...
public class SoundSystemLoader {
	 ...
	 * instance is specified, it will be used, otherwise this method will create a
	 * SoundSystem instance and return a handle to it if the XML file contains a
	 * CREATE tag. XML tags are NOT case sensitive. Parameters must be placed inside
	 * whichever tag they belong to (nested PARAMETER tags are not recognized).
	 * Parameter values must NOT contain whitespace (this includes filenames and
	 * sourcenames). Parameter values must NOT be inside quotes. Parameter names are
	 * NOT case sensetive, but parameter values ARE case sensitive. The supported
	 * tags and their parameters are listed below. Parameter names in brackets
	 * indicate they are optional. Parameter names without brackets are not
	 * optional. Whenever optional parameters are not defined, default values will
	 * be used instead. <br>
	 * <br>
	 * !-- <br>
	 * &nbsp;&nbsp; XML Comment <br>
	 ...
	 * @return Handle to SoundSystem, or null if none was initially specified and a
	 *         CREATE command was not found in the XML file.
	 */
			...
			if (command != null && !command.substring(0, 1).equals("/")) {
				...
				if (command.equals("ADDLIBRARY")) {
					...
					try {
						...
						if (verbose)
							message("Command:  SoundSystemConfig.addLibrary( " + commands.get("CLASSNAME")
									+ ".class );", 1);
						// Add the library
						SoundSystemConfig.addLibrary(c);
					} catch (ClassNotFoundException cnfe) {
						errorMessage("Unable to add library plug-in in " + "method 'loadXML':  Class name '"
								+ commands.get("CLASSNAME") + "' not found.");
					...
					try {
						...
						c = Class.forName(commands.get("CLASSNAME"));
						if (verbose)
							message("Command:  SoundSystemConfig.setCodec( \"" + commands.get("EXTENSION") + "\", "
									+ commands.get("CLASSNAME") + ".class );", 1);
						// Associate the codec with the correct extension:
						SoundSystemConfig.setCodec(commands.get("EXTENSION"), c);
					} catch (ClassNotFoundException cnfe) {
						errorMessage("Unable to set codec plug-in for " + "extension '" + commands.get("EXTENSION")
								+ "' in method 'loadXML':  Class name '" + commands.get("CLASSNAME") + "' not found.");
					} catch (SoundSystemException sse) {
						printStackTrace(sse);
					}
					...
					if (parameter != null && !parameter.equals("")) {
						try {
							...
							if (verbose) {
								...
								s = (SoundSystem) c.getDeclaredConstructor().newInstance();
							}
						} catch (ClassNotFoundException cnfe) {
						...
						if (verbose) {
							message("Unable to instantiate the Sound System " + "in method 'loadXML'  Returning null.",
									1);
							if (s != null)
								s.cleanup();
							s = null;
					...
					if (s == null) {
						errorMessage("Encountered 'loadSound' command " + "before SoundSystem was instantiated "
					...
					// Make sure the SoundSystem has been instantiated
					if (s == null) {
						errorMessage("Encountered 'newSource' command " + "before SoundSystem was instantiated "
								+ "in method 'loadXML'.  returning null.");
						return null;
					}
					...
					if (parameter != null && !parameter.equals("")) {
						...
						try {
							...
							errorMessage("Error parsing float 'X' from " + "String '" + parameter + "' in "
					...
					if (parameter != null && !parameter.equals("")) {
						...
						try {
							...
							errorMessage("Error parsing float 'Y' from " + "String '" + parameter + "' in "
					...
					if (parameter != null && !parameter.equals("")) {
						...
						try {
							...
							errorMessage("Error parsing float 'Z' from " + "String '" + parameter + "' in "
					...
					if (parameter != null && !parameter.equals("")) {
						...
						try {
							...
							errorMessage("Error parsing float " + "'DISTORROLL' from String '" + parameter
									+ "' in method " + "'loadXML'.  Using default value.");
							distOrRoll = 0;
					...
					if (sourcename.equals("")) {
						errorMessage("Parameter 'SOURCENAME' not " + "specified for 'NEWSOURCE' tag in "
								+ "method 'loadXML.  Unable to create " + "new source.");
					} else if (filename.equals("")) {
						errorMessage("Parameter 'FILENAME' not " + "specified for 'NEWSOURCE' tag in "
								+ "method 'loadXML.  Unable to create " + "new source.");
					} else {
						if (verbose)
							message("Command:  s.CommandQueue( new " + "paulscode.sound.CommandObject( "
									+ "paulscode.sound.CommandObject." + "NEW_SOURCE, " + priority + ", " + toStream
									+ ", " + toLoop + ", \"" + sourcename + "\", "
									+ "new paulscode.sound.FilenameURL( \"" + filename + "\" ), " + x + ", " + y + ", "
									+ z + ", " + attModel + ", " + distOrRoll + " ) );", 1);
						s.CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.NEW_SOURCE,
								priority, toStream, toLoop, sourcename, new paulscode.sound.FilenameURL(filename), x, y,
