...
public class EntityBoar extends EntityPig {
	...
	public EntityLiving getClosestTarget(Entity entity1, double d2) {
		...
		List<Entity> list7 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(d2, d2, d2));
	...
	public boolean attackEntityFrom(DamageSource source, int damage) {
		if(super.attackEntityFrom(source, damage)) {
			if(this.riddenByEntity != source.getEntity() && this.ridingEntity != source.getEntity()) {
				if(source.getEntity() != this && this.worldObj.difficultySetting > 0) {
	...
	protected void attackEntity(Entity entity1, float f2) {
		if((double)f2 < 2.5D && entity1.boundingBox.maxY > this.boundingBox.minY && entity1.boundingBox.minY < this.boundingBox.maxY) {
			this.attackTime = 20;
			entity1.attackEntityFrom(DamageSource.causeMobDamage(this), this.force);
	...
	public void destroyDrops(Entity entity1, double d2) {
		List<Entity> list4 = this.worldObj.getEntitiesWithinAABBExcludingEntity(entity1, entity1.boundingBox.expand(d2, d2, d2));
