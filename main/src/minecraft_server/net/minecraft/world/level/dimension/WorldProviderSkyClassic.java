...
public class WorldProviderSkyClassic extends WorldProvider {

	// This world provider provides stuff like classic fog, sky color, brightness table or 
	// grayscale lightmap for the never used sky world type found in beta.
	
	public WorldProviderSkyClassic() {
		this.hasSolidFloor = false;
	}
	...
	public int[] updateLightmap(float torchFlicker, float gamma, EntityPlayer thePlayer) {
		...
		if(world != null) {
			...
			for(int idx = 0; idx < 256; ++idx) {
				...
				lightmapColors[idx] = alpha << 24 | cI << 16 | cI << 8 | cB;
			}	
		}
	...
	public Vec3D getSkyColorBottom(Entity entity, float partialTicks) {
		return this.getSkyColor(entity, partialTicks);
	}
