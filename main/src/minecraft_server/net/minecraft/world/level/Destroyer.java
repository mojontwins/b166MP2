...
public class Destroyer {
	public static void DestroyBlast(World world, Entity entity, double posX, double posY, double posZ, float force, boolean fire) {
		...
		HashSet<ChunkPosition> hashSet10 = new HashSet<>();
		float f11 = force;
		byte b12 = 16;
		...
		List<Entity> list19 = world.getEntitiesWithinAABBExcludingEntity(entity, AxisAlignedBB.getBoundingBoxFromPool((double)i13, (double)i15, (double)i17, (double)i14, (double)i45, (double)i46));
		...
		for(int i21 = 0; i21 < list19.size(); ++i21) {
			...
			if(d23 <= 1.0D) {
				...
				if(!(entity49 instanceof EntityOgre)) {
					entity49.attackEntityFrom(DamageSource.explosion, (int)((d57 * d57 + d57) / 2.0D * 3.0D * (double)force + 1.0D));
		...
		force = f11;
		ArrayList<ChunkPosition> arrayList48 = new ArrayList<>();
		...
		for(i50 = arrayList48.size() - 1; i50 >= 0; --i50) {
			...
			i52 = chunkPosition51.x;
			i53 = chunkPosition51.y;
			...
			if(i54 > 0) {
				Block.blocksList[i54].dropBlockAsItemWithChance(world, i52, i53, i26, world.getBlockMetadata(i52, i53, i26), 0.3F, 0);
