...
public class WorldGenDungeons extends WorldGenerator {
	...
	protected List<LootItem> possibleLootItems = new ArrayList<>();
	...
	public WorldGenDungeons(BiomeGenBase biomeGen) {
		...
		this.possibleLootItems.add(new LootItem (Item.saddle.shiftedIndex, 0, 1, false, 0));
		this.possibleLootItems.add(new LootItem (Item.ingotIron.shiftedIndex, 0, 4, false, 0));
		this.possibleLootItems.add(new LootItem (Item.bread.shiftedIndex, 0, 1, false, 0));
		this.possibleLootItems.add(new LootItem (Item.wheat.shiftedIndex, 0, 6, false, 0));
		this.possibleLootItems.add(new LootItem (Item.gunpowder.shiftedIndex, 0, 4, false, 0));
		this.possibleLootItems.add(new LootItem (Item.silk.shiftedIndex, 0, 4, false, 0));
		this.possibleLootItems.add(new LootItem (Item.bucketEmpty.shiftedIndex, 0, 1, false, 0));
		this.possibleLootItems.add(new LootItem (Item.appleGold.shiftedIndex, 0, 1, true, 100));
		this.possibleLootItems.add(new LootItem (Item.diamond.shiftedIndex, 0, 1, true, 25));
		this.possibleLootItems.add(new LootItem (Item.redstone.shiftedIndex, 0, 4, true, 2));
		this.possibleLootItems.add(new LootItem (Item.record13.shiftedIndex, 0, 1, false, 0));
		this.possibleLootItems.add(new LootItem (Item.recordCat.shiftedIndex, 0, 1, false, 0));
	}
	
	// This method checks if the selected random location is valid for this feature to spawn.
	public boolean validLocation(World world, int x, int y, int z, int width, int height, int depth) {
		int exposedCeilingBlocks = 0;
		int accesses = 0;
		
		// Iterate a box 1 block wider than the decided WxHxD
		for(int xx = x - width - 1; xx <= x + width + 1; ++xx) {
			for(int yy = y - 1; yy <= y + height + 1; ++yy) {
				for(int zz = z - depth - 1; zz <= z + depth + 1; ++zz) {
					...
					// Dungeon must spawn on a solid surface, no gaps.
					if(yy == y - 1 && !material.isSolid()) {
						return false;
					}

					// Count how many exposed ceiling blocks.
					if(yy == y + height + 1 && !material.isSolid()) {
						//return false;
						exposedCeilingBlocks ++;
					}

					// y is ground level. Count how many surrounding blocks have a 2 blocks tall "gap" (air or water).
			...
			for(int yy = y + height; yy >= y - 1; --yy) {
				for(int zz = z - depth - 1; zz <= z + depth + 1; ++zz) {
					if(xx != x - width - 1 && yy != y - 1 && zz != z - depth - 1 && xx != x + width + 1 && yy != y + height + 1 && zz != z + depth + 1) {
						// If not in the borders, clear with air.
						...
						// If in the borders, substitute solid by cobble
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		if(!this.validLocation(world, x, y, z, width, height, depth)) return false;
		
		this.drawRoom(world, rand, x, y, z, width, height, depth);
		...
		for(int i = 0; i < 2 + (this.maxDungeonTier > 3 ? 1 : 0); ++i) {
			for(yy = 0; yy < 3; ++yy) {
				...
				if(world.getBlockId(xx, y, zz) == this.getFillerId()) {
					...
					if(adjacentSolidBlocks == 1 && chests < maxChests) {
						...
						if(tileEntityChest != null) {
							...
							world.setBlockWithNotify(xx, y, zz, this.getFillerId());
						}
					}
				}
			}
		...
		this.extraDecos(world, x, y, z, rand);
	...
	private ItemStack pickChestLootItem(Random rand) {	
		...
		int selection = rand.nextInt (possibleLootItems.size() + 2);
		if (selection < possibleLootItems.size()) {
			LootItem item = possibleLootItems.get(selection);
